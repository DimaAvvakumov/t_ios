...
- (NSString *) errorStrigByNumber: (NSInteger) errNO {
    switch (errNO) {
        case ServerParams_ErrorAuthorizationFailed: {
            return ALLocalizedStringGeneral(@"ErrorAuthorizationFailed", @"Ошибка авторизации");
            
            break;
        }
        case ServerParams_ErrorAccessDeny: {
            return ALLocalizedStringGeneral(@"ErrorAccessDeny", @"Нет доступа");
            
            break;
        }
        case ServerParams_ErrorServerWrongResponse: {
            return ALLocalizedStringGeneral(@"ErrorServerWrongResponse", @"Сервер временно недоступен. Пожалуйста повторите попытку позже");
            
            break;
        }
        case ServerParams_ErrorServerIsUnavailable: {
            return ALLocalizedStringGeneral(@"ErrorServerIsUnavailable", @"Сервер недоступен. Пожалуйста повторите попытку позже");
            
            break;
        }
        case ServerParams_ErrorUnknow: {
            return ALLocalizedStringGeneral(@"ErrorUnknow", @"Неизвестная ошибка. Пожалуйста повторите попытку позже");
            
            break;
        }
        case ServerManager_ErrorLoginFailed: {
            return ALLocalizedStringGeneral(@"ErrorLoginFailed", @"Неверное имя пользователя или пароль");
            
            break;
        }
        case ServerParams_ErrorFileManagerCreate: {
            return ALLocalizedStringGeneral(@"ErrorFileManagerCreate", @"Ошибка файловой системы. Пожалуйста, убедитесь что у вас достаточно свободного места");
            
            break;
        }
        case ServerParams_ErrorFileManagerModify: {
            return ALLocalizedStringGeneral(@"ErrorFileManagerModify", @"Ошибка файловой системы. Пожалуйста, повторите попытку позже");
            
            break;
        }
        default:
            return ALLocalizedStringGeneral(@"Check_internet_and_try_later", @"Сервер недоступен. Пожалуйста, проверьте ваше сетевое соединение или повторите попытку позже");
